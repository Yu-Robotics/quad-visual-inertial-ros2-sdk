    ...
    if (desc.idVendor == vid && desc.idProduct == pid) {
      ...
      for (unsigned int j = 0; j < conf_desc->bNumInterfaces; j++)
      {
        ...
        if (interface->num_altsetting == 1)
        {
          ...
          if (if_desc->bInterfaceClass == 0xff && if_desc->bInterfaceSubClass == 0x00 && if_desc->bInterfaceProtocol == 0x00) {
            LIBUSB_CHECK(libusb_open(device_list[i], &device));
            char name[64];
            libusb_get_string_descriptor_ascii(device, if_desc->iInterface, (unsigned char*)name, sizeof(name) - 1);
...
std::string uvc_read_calibration(int vid, int pid, const std::string& name)
{
  ...
  // read file
  buffer[0] = 'R';
  usb_bulk_write(buffer, 1);
  usb_bulk_read(&length, 4);
  length = le32toh(length);
  uint32_t index = 0;
  while (index < length) {
    int len = usb_bulk_read(buffer + index, std::min(length - index, payload_d2h));
    index += len;
  }
  usb_device_close();

  uint32_t crc = le32toh(*(uint32_t*)(buffer + length - sizeof(crc)));
  length -= sizeof(crc);
  buffer[length] = '\0';
  uint32_t hash = crc32(buffer, length);
  if (hash != crc) {
    fprintf(stderr, "uvc: read crc 0x%x != 0x%x\n", hash, crc);
    return {};
  }
